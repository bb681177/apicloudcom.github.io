(function($, hljs){
    ...
    $(document).ready(function(){
        if($('.content').length){
            ...
            //copy sample code
            $('.content pre').each(function(index, el){
        ...
        //get release IDE
        $.ajax({
            url: 'http://module.apicloud.com/getReleaseApicloudIde',
        ...
        /*
         *sort排序是按content下所有所有子文件夹一起排序
         */
        var $menu = $('.container-fluid .menu .category');
        ...
        //toggle subMenu
        $('.menu .second').each(function(index, el) {
            ...
            $title.on('click',function(){
                ...
                for(i; i<len; i++){
                    if($title.text().trim() === titArr[i]){
                        var thisPath = pathArr[i];

                        //submenu outline
                        $('section.content').load('/subMenuOutline/'+ thisPath, function(data) {
        ...
        $.ajax({
            ...
            success: function(data){
                if(data.status){
                    ...
                    // var txt = data.result.userName +"| 开发控制台";
        ...
        if($('#vplayer').length){
            ...
            var $vplayer = $('#vplayer .wrap');

            var myvideo;

            $vCourse.on('click', 'a', function(event){
            ...
            $("#vplayer").on("click", '.close', function(){
                if(myvideo) {
                    myvideo.pause();
                    myvideo.dispose();
                    myvideo=null;
            ...
            $vCourse.on('click', '.list-toggle', function(event) {
                $vCourse.toggleClass('active');
                event.preventDefault();
            });
        ...
        if(searchStr === 'from=home'){
            var $activeLi = $('.menu .second .active');
        ...
        if($downloadCon.length){
            ...
            $.ajax({
                url: 'http://module.apicloud.com/getApicloudSdk?length=2&platform='+platform,
            ...
            $.ajax({
                url: 'http://module.apicloud.com/getSublimeAndWebstorm',
                // url: 'http://192.168.13.183:81/getApicloudSdk?length=10',
                dataType: 'jsonp',
                success: function(data){
                    if(data.status){
            ...
            $.ajax({
                url: 'http://module.apicloud.com/getMdSdk?length=2',
            ...
            /*apploader*/
            $.ajax({
                url: 'http://module.apicloud.com/getLoader',
        ...
        if($vCourse.length){
            $.ajax({
                url: 'http://module.apicloud.com/getTypeVideo',
                ...
                success: function(data){
                    if(data.status){
                        ...
                        $('.v-course .get-more').click(function(){
                            ...
                            $.ajax({
                                url: 'http://module.apicloud.com/getVideo?length='+getLen+'&type='+type+'&start='+start,
        ...
        if($expCode.length){
            $.ajax({
                url: 'http://module.apicloud.com/getCaseVideo?length=100',
    ...
    if($('#gplayer').length){
        ...
        $.ajax({
            url: 'http://www.apicloud.com/getOfficialLoader?doc=1',
            dataType: 'jsonp',
            success:function(data) { 
            console.log(data);
                var data = data.result;
                var wxUrlAnd = data[0].upkApkUrl;
                console.log(wxUrlAnd);
                var wxUrlIos = 'http://www.apicloud.com/getIpa/'+data[0].upkToken;
                // console.log(data[0].upkApkUrl);
                var weixinEleAnd = $('.Qr_code .android-img');
                var weixinEleIos = $('.Qr_code .ios-img');
                var hostname = window.location.host,
                wxImg = hostname+'/img/icon-wx.png';
                payQrcode(weixinEleAnd,wxUrlAnd,wxImg,110,110);
                payQrcode(weixinEleIos,wxUrlIos,wxImg,110,110);
            }, 
        })
        
        function payQrcode(el,url,imgUrl,width,height){
                var $this = el,
                // size = size || 95;
                height = height,
                width = width,
